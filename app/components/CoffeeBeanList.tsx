  ...
  flavorWheelData.categories.forEach((level1) => {
    level1.children.forEach((level2) => {
      level2.children.forEach((level3) => {
        map[level3.id] = { name: level3.name, color: level3.color_hex };
      });
    });
  });
  return map;
...
const CoffeeBeanList = ({ beans, selectedFlavors = [] }: CoffeeBeanListProps) => {
  // Show popular beans if no filter is applied
  const beansToDisplay = selectedFlavors.length === 0 ? getPopularBeans(beans) : beans;

  // Group beans by coffee shop
  const beansByShop = beansToDisplay.reduce(
    (acc, bean) => {
    ...
    <div>
      ...
      {beansToDisplay.length === 0 ? (
        <div className="bg-gray-50 rounded-lg border border-gray-200 p-5">
          <p className="text-gray-600">No coffee beans match the selected flavors.</p>
        </div>
      ) : (
        <div className="space-y-4">
          {shopsWithBeans.map((shop) => (
            <div key={shop.id} className="rounded-lg border border-gray-200 overflow-hidden">
              {/* Level 2: Coffee Shop Name */}
              <div className="bg-gray-50 border-b border-gray-200 px-5 py-3">
                <h3 className="text-sm font-semibold uppercase tracking-wide text-gray-600">{shop.name}</h3>
              </div>
              
              {/* Level 3 & 4: Beans and Pills */}
              <ul className="divide-y divide-gray-100">
                {beansByShop[shop.id].map((bean) => (
                  <li key={bean.id} className="px-5 py-4 hover:bg-gray-50 transition-colors">
                    <Link href={`/coffee-bean/${bean.id}`} className="text-base font-semibold text-gray-900 hover:text-blue-700 hover:underline transition-colors">
                      {bean.name}
                    </Link>
